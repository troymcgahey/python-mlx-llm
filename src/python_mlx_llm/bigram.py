...
def main() -> None:
    ...
    current_id = char_to_id["h"]
    neural_generated_characters = [id_to_char[current_id]]

    for step in range(30):
        current_input = mx.array([current_id])
        next_logits = model(current_input)[0]

        next_id = mx.random.categorical(next_logits).item()
        next_character = id_to_char[next_id]

        neural_generated_characters.append(next_character)
        current_id = next_id

    neural_generated_text = "".join(neural_generated_characters)

    print(
        "Neural model generated:",
        repr(neural_generated_text),
    )
